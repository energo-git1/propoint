    ...
    try {
      if (isDetailPage()) {
        ...
        if (r._skip) {
          ...
          appendLog(`<br>✅ <strong>Užduotis importuota!</strong>`);

          // ── Dokumentai ────────────────────────────────────────
          const docCount = await extractAndUploadDocs(task.id, appendLog);
          if (docCount > 0) {
            appendLog(`📎 Įkelta dokumentų: <strong>${docCount}</strong>`);
          }
